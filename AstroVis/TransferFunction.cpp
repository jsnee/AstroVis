...
void CTransferFunction::render(int offset, int id) {
	...
	glDisable(GL_BLEND);
    int m_pTrapStart1 = m_pTrapezoids[0].getStart();
    int m_pTrapEnd1 = m_pTrapezoids[0].getEnd();
	...
	glColor4f(1.0,1.0,1.0,1.0);
    if (scaleColorMap == 1) {
        glTexCoord1f(0.0); glVertex2i(m_pTrapStart1, 275);
        glTexCoord1f(1.0); glVertex2i(m_pTrapEnd1, 275);
        glTexCoord1f(1.0); glVertex2i(m_pTrapEnd1, 265);
        glTexCoord1f(0.0); glVertex2i(m_pTrapStart1, 265);
    } else {
...
void CTransferFunction::updateColorTable(int nStart, int nEnd) {
	...
	CVector color;
	int nIndex = nStart * 4;
    double scalar = 255.0 / (nEnd - nStart);
    int visibleSteps = 0 - scalar;    
	...
	for(int nStep = nStart; nStep <= nEnd; nStep++) {
		...
		for(int p = 0; p < m_nNumTrapezoids; p++) {
			...
			dAlphaSum += dValue;
			if (dValue > 0.0) {
				color.set(r[visibleSteps]/255.0, g[visibleSteps]/255.0, b[visibleSteps]/255.0, 0.0);//= m_pTrapezoids[p].color();
...
void CTransferFunction::updateColor() {
    for(int i = 0; i < m_nNumTrapezoids; i++) {
        updateColorTable(m_pTrapezoids[i].getStart(), m_pTrapezoids[i].getEnd());
    }
}